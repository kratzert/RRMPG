...
"""Interface to the IceMelt + Cemaneige Hysteresis + GR4J coupled hydrological model."""
...
from .cemaneigehystgr4jice_model import run_cemaneigehystgr4jice
...
class CemaneigeHystGR4JIce(BaseModel):
    """Interface to the IceMelt + Cemaneige Hysteresis + GR4J coupled hydrological model.
    
    This class builds an interface to the coupled model, consisting of the IceMelt model [1], the
    Cemaneige snow routine with hysteresis [2][3] and the GR4J model [4]. This model should only 
    ...
    [3] Riboust, P., Thirel, G., Le Moine, N., Ribstein, P. "Revisiting a simple degree-day model for
    ...
    [4] Perrin, Charles, Claude Michel, and Vazken Andréassian. "Improvement 
    ...
    _param_list = ['CTG', 'Kf', 'Thacc', 'Rsp', 'x1', 'x2', 'x3', 'x4', 'DDF']
    ...
    def __init__(self, params=None):
        """Initialize a IceMelt + Cemaneige Hysteresis + GR4J coupled hydrological model object.
    ...
    def simulate(self, prec, mean_temp, min_temp, max_temp, etp, frac_ice,
        ...
        Returns:
            An array with the simulated stream flow and optional arrays for 
            the two storages. 
        ...
        if return_storages:
            ...
            snowmelt = np.zeros((prec.shape[0], params.size), np.float64)
        ...
        for i in range(params.size):    
            if return_storages:
                (qsim[:, i], 
                 ...
                 ice_melt[:, i],
                 snowmelt[:, i],
                 rain[:, :, i]) = run_cemaneigehystgr4jice(prec, mean_temp, etp, frac_ice, 
                                                    frac_solid_prec,
                                                    snow_pack_init, 
                                                    thermal_state_init,
                                                    sca_init,
                ...
                qsim[:, i], _, _, _, _, _, _, _, _ = run_cemaneigehystgr4jice(prec, mean_temp, etp, frac_ice,
                                                           frac_solid_prec, 
                                                           snow_pack_init,
                                                           thermal_state_init,
                                                           sca_init,
        ...
        if return_storages:
            return qsim, G, eTG, s_store, r_store, sca, ice_melt, snowmelt, rain
        else:
            return qsim
    ...
    def fit(self, obs, prec, mean_temp, min_temp, max_temp, etp, frac_ice,
        ...
        """Fit the IceMelt + Cemaneige Hysteresis + GR4J coupled hydrological model to a observed timeseries
        ...
        if not isinstance(r_init, numbers.Number):
            raise TypeError("'r_init' must be a Number.")
        # Check if frac_ice is a 1D array
        if isinstance(frac_ice, np.ndarray) and frac_ice.ndim != 1: 
            raise ValueError("frac_ice must be a 1D array.")
        ...
        args = (obs, prec, mean_temp, frac_solid_prec, etp, frac_ice, snow_pack_init,
    ...
    def fit_Q_SCA(self, obs, prec, mean_temp, min_temp, max_temp, etp, frac_ice, NDSI1, NDSI2, NDSI3, NDSI4, NDSI5,  
            met_station_height, loss_metric="mse", snow_pack_init=0, thermal_state_init=0, sca_init=0,
            s_init=0, r_init=0, altitudes=[]):
        """Fit the IceMelt + Cemaneige Hysteresis + GR4J coupled hydrological model to a observed timeseries
            using discharge as well as SCA as target.
        ...
        Args:
            ...
            etp: Array of mean potential evapotranspiration [mm]
            frac_ice: Array of the fraction of glaciated area in each elevation band. [0 - 1]
            NDSI1: Array of Normalized Difference Snow Index (NDSI) values for layer 1.
            NDSI2: Array of Normalized Difference Snow Index (NDSI) values for layer 2.
            NDSI3: Array of Normalized Difference Snow Index (NDSI) values for layer 3.
            NDSI4: Array of Normalized Difference Snow Index (NDSI) values for layer 4.
            NDSI5: Array of Normalized Difference Snow Index (NDSI) values for layer 5.
            met_station_height: Height of the meteorological station [m]. 
                Needed to calculate the fraction of solid precipitation and
                optionally for the extrapolation of the meteorological inputs.
            loss_metric: (optional) The loss metric allows to choose between
            ...
            thermal_state_init: (optional) Initial value of the thermal state
                of the snow pack
            sca_init: (optional) Initial value of the snow-covered area fraction.
            s_init: (optional) Initial value of the production storage as 
                fraction of x1. 
            r_init: (optional) Initial value of the routing storage as fraction
                of x3.
        ...
        Raises:
            ValueError: If one of the inputs contains invalid values or if `frac_ice` is not a 1D array.
            TypeError: If one of the inputs has an incorrect datatype.
            RuntimeErrror: If there is a size mismatch between the 
        ...
        # pack input arguments for scipy optimizer
        args = (obs, prec, mean_temp, frac_solid_prec, etp, frac_ice, NDSI1, NDSI2, NDSI3, NDSI4, NDSI5, snow_pack_init,
                thermal_state_init, sca_init, s_init, r_init, self._dtype, loss_metric)
        bnds = tuple([self._default_bounds[p] for p in self._param_list])
...
def _loss(X, *args):
    ...
    outflow, _, _, _, _, _, _, _, _ = run_cemaneigehystgr4jice(prec, mean_temp, etp, frac_ice,
    ...
    if loss_metric == "mse":
        ...
        loss_value = 1 - calc_kge(obs, outflow)
    else:
...
def _loss_Q_SCA(X, *args):
    ...
    NDSI4 = args[9]
    NDSI5 = args[10]
    snow_pack_init = args[11]
    thermal_state_init = args[12]
    sca_init = args[13]
    s_init = args[14] 
    r_init = args[15] 
    dtype = args[16]
    ...
    # Calcuate simulated outflow
    outflow, _, _, _, _, sca, _, _, _ = run_cemaneigehystgr4jice(prec, mean_temp, etp, frac_ice,
                                            frac_solid_prec, snow_pack_init, 
                                            thermal_state_init, sca_init, s_init, r_init,
                                            params[0])
